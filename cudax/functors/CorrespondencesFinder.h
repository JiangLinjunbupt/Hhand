...
struct CorrespondencesFinder {
	...
	__device__ void projection_segment(const glm::vec3 & p, 
	...
	__device__ void projection_segments(const glm::vec3 & p, 
	...
	__device__ void projection_convsegment(const glm::vec3 & p, const glm::vec3 & c1, const glm::vec3 & c2, float r1, float r2, int index1, int index2,
		...
		float omega = sqrt(length_x2 - delta_r * delta_r);   //求正切的另一条边，参加两个圆外切线的画法
		float beta = length(p - t) * delta_r / omega;
		s = t - beta * x / length_x;

		if (is_point_on_segment(s, c1, c2)) {
			float gamma = delta_r * length(c2 - t + beta * x / length_x) / length_x;//用相似关系计算
	...
	__device__ void projection_convtriangle(const glm::vec3 & p, 
		...
		const glm::vec3 & u1, const glm::vec3 & u2, const glm::vec3 & u3, const glm::vec3 & m, 
