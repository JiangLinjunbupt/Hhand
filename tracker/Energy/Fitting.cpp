...
void energy::Fitting::init(Worker *worker) {
	...
	kernel_init(this->settings, 
		camera->width(), 
		camera->height(), 
		num_thetas, 
		camera->focal_length_x(), 
		camera->focal_length_y(),
		camera->inv_projection_matrix().data(),
		d, model->centers.size(),
		model->blocks.size(),
		model->max_num_outlines, 
		model->num_tangent_fields,
		model->num_outline_fields, 
		false,
		worker->test, 
		worker->model->model_type);
	CHECK_ERROR_GL();
}
...
void energy::Fitting::track(DataFrame& frame, LinearSystem& sys, bool rigid_only, bool eval_error, float & push_error, float & pull_error, int iter) {
	...
	kernel_upload_model(d, 
		model->centers.size(), model->blocks.size(), model->outline_finder.outline3D.size(), 
		model->num_tangent_fields, model->num_outline_fields,
	...
	if (last_uploaded_id != frame.id) {
		...
		if (settings->fit2D_enable) {
			cv::flip(sensor_silhouette, sensor_silhouette_flipped, 0 /*flip rows*/); //此处绕x轴反转可能是因为计算得到的render图像是反转的
