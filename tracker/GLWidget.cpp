...
void GLWidget::keyPressEvent(QKeyEvent *event) {
	...
	case Qt::Key_1: {
		...
		worker->model->resize_model(1.05f, 1.0f, 1.0f);
	}
	break;
	case Qt::Key_2: {
		cout << "uniform scaling down" << endl;
		worker->model->resize_model(0.95f, 1.0f, 1.0f);
	}
	break;
	case Qt::Key_3: {
		cout << "width scaling up" << endl;
		worker->model->resize_model(1.0f, 1.05f, 1.0f);
	}
	break;
	case Qt::Key_4: {
		cout << "width scaling down" << endl;
		worker->model->resize_model(1.0f, 0.95f, 1.0f);
	}
	break;
	case Qt::Key_5: {
		cout << "thickness scaling up" << endl;
		worker->model->resize_model(1.0f, 1.0f, 1.05f);
	}
	break;
	case Qt::Key_6: {
		cout << "thickness scaling down" << endl;
		worker->model->resize_model(1.0f, 1.0f, 0.95f);
	}
	break;
	}
}
