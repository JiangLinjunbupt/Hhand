...
bool intersect_circle_line(const glm::dvec2 & c, double r, double slope, double intercept, glm::dvec2 & t1, glm::dvec2 & t2) {
	...
	double centery = c[1];
	if (abs(slope) < 100) {
		// from the law of cosines  //圆的方程和直线的方程联合求解
		...
		double x1, x2;
		if (!quadratic_roots(a, b, d, x1, x2)) return false;    //判断 一元二次方程 求解条件b^2 - 4ac
		else {
...
bool intersect_segment_segment(const glm::dvec2 & a, const glm::dvec2 & b, const glm::dvec2 & c, const glm::dvec2 & d, glm::dvec2 & t) {
	...
	double denom = (x1 - x2)*(y3 - y4) - (y1 - y2)*(x3 - x4);    //二维向量叉乘：可以判断向量a,b的左右关系，或着重合；几何意义表示的是以ab为边的平行四边形面积；
	if (abs(denom) < eps) return false;
	//公式的推导：https://blog.csdn.net/wangjichen_1/article/details/5969794
...
std::vector<std::pair<glm::dvec2, glm::dvec2>> intersect_segment_segment_same_circle(const glm::dvec2 & c, double r, const glm::dvec2 & s1, const glm::dvec2 & e1, const glm::dvec2 & s2, const glm::dvec2 & e2) {
	...
	//std::cout << u2[0] << "    " << u2[1] << std::endl;

	//std::cout << "r = " << r << std::endl;
	//std::cout << "end\n";

	std::vector<std::pair<double, double>> arcs1;              //这里就是看从开始到结束，能否通过一次逆时针旋转得到
	if (alpha1 < beta1)
	...
	//{
	//	std::cout << "first : " << intersections[i].first[0] - c[0] << "   " << intersections[i].first[1] - c[1]<<std::endl
	//		<< "second : " << intersections[i].second[0] - c[0] << "   " << intersections[i].second[1] - c[1] << std::endl;
	//}
...
bool get_tangents(const glm::dvec3 & c1, const glm::dvec3 & c2, double r1, double r2,
	...
	if (d2 - delta_r2 < 0) return false;               //小于则代表没有一个圆包含在另一个圆内部，没有切线
