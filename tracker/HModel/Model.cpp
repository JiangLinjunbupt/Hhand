...
Model::Model() :outline_finder(this), serializer(this), semantics(this) {
	...
	std::cout << "phalanges size : " << phalanges.size() << std::endl;
...
void Model::manually_adjust_initial_transformations() {
	Mat3f R;
	// thumb
	R = build_rotation_matrix(Vec3f(-1.45f, 0.6f, -1.95f));
	...
	// index	
	R = build_rotation_matrix(Vec3f(0, 0, -0.08f));
