...
struct OutlineTraverser {
	...
	void store_tangents(const int circle_id_1, const int circle_id_2, const glm::dvec2 & point_1, const glm::dvec2 & point_2, const size_t block_id, size_t & point_id) {
		OutlinePoint point;

		point.value = point_1;
		point.i1 = circle_id_1;    //属于哪个圆
		point.type1 = 0;
		point.i2 = segments.size();
		point.type2 = 1;
		points.push_back(point);

		circles[circle_id_1].points.push_back(point_id++);
		circles[circle_id_1].center = glm::dvec2(model->centers[circle_id_1][0], model->centers[circle_id_1][1]);
		circles[circle_id_1].radius = model->radii[circle_id_1];
		circles[circle_id_1].block = block_id;

		point.value = point_2;
		point.i1 = circle_id_2;
		point.type1 = 0;
		point.i2 = segments.size();
		point.type2 = 1;
		points.push_back(point);

		circles[circle_id_2].points.push_back(point_id++);
		circles[circle_id_2].center = glm::dvec2(model->centers[circle_id_2][0], model->centers[circle_id_2][1]);
		circles[circle_id_2].radius = model->radii[circle_id_2];
		circles[circle_id_2].block = block_id;

		OutlineSegment segment;
		segment.t1 = point_1;
		segment.t2 = point_2;
		segment.indices = glm::dvec2(circle_id_1, circle_id_2);
		segment.points.push_back(point_id - 2);
		segment.points.push_back(point_id - 1);
	...
	size_t find_starting_point(size_t & k, size_t & i) {
		...
		double min_delta = std::numeric_limits<double>::max();
		glm::dvec2 v = p - circles[i].center;     //这里的v就是 r*(0,1)
		double alpha_before = myatan2(v);         //这里计算返回 1.57弧度 = 90°

		double alpha, beta, delta; glm::dvec2 u;
		for (size_t j = 0; j < circles[i].points.size(); j++) {       //寻找顺时针旋转，在圆上距离up这个向量最近的点作为starting_point  //最开始我以为是逆时针，这个自己输出一下，看看值就行了
			alpha = alpha_before;
		...
		//cout << "Final is： "<<(points[k].value - circles[i].center)[0] << "  " << (points[k].value - circles[i].center)[1] << endl;
		return k;
	}
	...
	for (size_t f = 0; f < model->fingers_block_indices.size(); f++) {	
		...
		if (palm_index != -1 && finger_index != -1) {
			intersections = intersect_segment_segment_same_circle(glm::dvec2(model->centers[palm_outline[palm_index].indices[0]][0], model->centers[palm_outline[palm_index].indices[0]][1]),
				model->radii[palm_outline[palm_index].indices[0]], 
				palm_outline[palm_index].start, palm_outline[palm_index].end,
