...
void HandFinder::binary_classification(cv::Mat& depth, cv::Mat& color) {    
   ...
   /* cv::Scalar hsv_min = settings->hsv_min;
    ...
    Scalar wband_size = _settings.wband_size;  //30
    Scalar depth_range= _settings.depth_range;  //150
    ...
    // TIMED_BLOCK(timer,"Worker_classify::(convert to HSV)")
    {
		...
		cv::inRange(color_hsv, settings->hsv_min, settings->hsv_max, /*=*/ mask_wristband);
    ...
    _wband_center = Vector3(0,0,0);  //也就是wrist的3D点云，将腕带上的蓝色点深度点，反投影到世界坐标的点云中。
