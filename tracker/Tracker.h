...
public:
	...
	OnlinePeformanceMetrics online_performance_metrics;        //暂时可以不管，save_rastorized_model为True才会用到
...
public:
	Tracker(Worker*worker, double FPS, std::string data_path, bool real_color) : worker(worker) {
		setSingleShot(false);  //若为true则定时器只执行一次，这显然不是想要的。
	...
	void process_track() {
		...
		//TICTOC_BLOCK(fetching_time, "Sensor fetch")
		{
			...
			if (mode == BENCHMARK) {
				...
				//std::cout << worker->handfinder->sensor_silhouette.rows << "  " << worker->handfinder->sensor_silhouette.cols << std::endl;
				...
				//cout << worker->handfinder->num_sensor_points << endl;
				if (current_frame == 1) {
					Vector3 translation = worker->trivial_detector->exec(worker->current_frame, worker->handfinder->sensor_silhouette);     //没看懂
					std::vector<float> thetas = worker->model->get_theta(); thetas[9] = 0; thetas[10] = 0;
					thetas[0] += translation[0]; thetas[1] += translation[1]; thetas[2] += translation[2];
					worker->model->move(thetas);
					worker->model->update_centers();
