...
namespace Eigen{
	inline QDebug operator<< (QDebug d, const Eigen::AlignedBox3f & box) { return d << "[" << box.min().x() << box.min().y() << box.min().z() << "]" << "[" << box.max().x() << box.max().y() << box.max().z() << "]"; }
	inline QDebug operator<< (QDebug d, const Eigen::AlignedBox2f & box) { return d << "[" << box.min().x() << box.min().y() << "]" << "[" << box.max().x() << box.max().y() << "]"; }
